...
public class AddBaby extends AppCompatActivity implements CompoundButton.OnCheckedChangeListener {
...
//            Log.i("TEST","Picked Date year is:"+pickedDate);
            ...
            if (Util.isInternetConnected(AddBaby.this)) {


                babyDetails();
            } else {
                Toast.makeText(AddBaby.this, "Please connect to Internet and Try Again", Toast.LENGTH_LONG).show();
            }
            ...
            db.collection("user").document(firebaseUser.getUid()).collection("baby").document(baby.docId).set(baby).addOnCompleteListener(this, new OnCompleteListener<Void>() {
                ...
                public void onComplete(@NonNull Task<Void> task) {
                    if (task.isComplete()) {
                        ...
                        Toast.makeText(AddBaby.this, "Updation Finished", Toast.LENGTH_LONG).show();
                        finish();
                    } else {
                        Toast.makeText(AddBaby.this, "Updation Failed", Toast.LENGTH_LONG).show();
                    }
                }
            });
...
//                                 babyDetails();
                            ...
                            db.collection("user").document(firebaseUser.getUid()).collection("baby").document(babyId).set(baby.docId).addOnCompleteListener(AddBaby.this, new OnCompleteListener<Void>() {
                                @Override
                                public void onComplete(@NonNull Task<Void> task) {
    ...
    public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
        ...
        switch (id){
            ...
            case R.id.radioButtonMale:
                if(isChecked){
                    Toast.makeText(AddBaby.this, "You Selected Male", Toast.LENGTH_LONG).show();
                    baby.gender="Male";
                }
                break;
            case R.id.radioButtonFemale:
                if(isChecked){
                    Toast.makeText(AddBaby.this, "You Selected Female", Toast.LENGTH_LONG).show();
                   baby.gender="Female";
                }
                break;
        }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if(id==101){
            Intent intent=new Intent(AddBaby.this,AllBabiesActivity.class);
